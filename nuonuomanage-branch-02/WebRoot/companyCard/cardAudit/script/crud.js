...
function exportList() {
    if (type == "2") {
        var nameOrTax=$('#nameOrTax').searchbox("getValue");
        if (nameOrTax.length < 4) {
            $.messager.alert("操作提示", "企业名称大于4位!");
        } else {
            $.ajax({
                url: "companyCard/cardAudit/exportMatch.action?nameOrTax=" + encodeURIComponent(nameOrTax),
                type: 'POST',
                success: function (res) {
                    if (res == "error") {
                        $.messager.alert("提示", "导出条数超出最大限制！");
                    } else {
                        window.open("companyCard/cardAudit/exportMatch.action?nameOrTax=" + encodeURIComponent(nameOrTax));
                    }
                }
            });
        }
